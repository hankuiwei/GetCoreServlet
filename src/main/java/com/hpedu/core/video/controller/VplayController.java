...
//	@Value("${page.limit}")
	 ...
	 * 点击播放视频（常规）
	 * */
	@RequestMapping("/general/general.html")
	public void toGenPlayer(HttpServletRequest req,HttpSession session,String classNo,String className,Model model)throws Exception{
	 ...
	 * 点击播放视频（常规）
	 * */
	@RequestMapping("/public/generalCourseDetail")
	public void toGeneralCourseDetail(HttpServletRequest req,HttpSession session,String classNo,String className,Model model)throws Exception{
		this.editUserInSession(session);
		session.setAttribute("queryString", "classNo="+classNo+"&className="+className);
		session.setAttribute("requestUrl", "public/generalCourseDetail");
		model= getMode(model,session,classNo,className,false);
	}
	/**
	 * 点击播放视频（常规）
	 * */
	@RequestMapping("/public/generalCourseDetail_more")
	public void toGeneralCourseDetail_more(HttpServletRequest req,HttpSession session,String classNo,String className,Model model)throws Exception{
		this.editUserInSession(session);
		session.setAttribute("requestUrl", "public/generalCourseDetail_more");
		session.setAttribute("queryString", "classNo="+classNo+"&className="+className);
		...
		if(classNo!=null&&!"".equals(classNo)){
			try{
			 ...
			 if(isMore){//有多个子视频
				 ...
				 map.put("pid", classNo);
				 if(StringUtils.isNotEmpty(generalVideo.getWeekVal())){
				 ...
				 if(StringUtils.isNotEmpty(generalVideo.getWeekVal())){
					//检查是否要开启页面定时
			    	 ...
			    	 if(edit_week[0].equals(curr_week[0]) && date.getHours() < Integer.parseInt(edit_week[1]) ){
			    		 Date editDate_now=new SimpleDateFormat("yyyy-MM-dd HH").parse(dateArr[0]+" "+edit_week[1]);
			    		 VideoChild lastVC = vclist.size()==0 ? null : vclist.get(vclist.size()-1);//什么鬼操作!!!2018年4月18日13:54:17 
			    		 if(lastVC==null||lastVC.getEditDate().getTime()<editDate_now.getTime()){
			    			     model.addAttribute("editTimerNum", editDate_now.getTime()-date.getTime());//更新下一集的毫秒数
			    			 if(lastVC!=null){
	 ...
	 * 点击播放视频（常规-专题-多关联视频）
	 * */
	@RequestMapping("/general/generalMore.html")
	public void toGenPlayerMore(HttpServletRequest req,HttpSession session,String classNo,String className,Model model,String vcid)throws Exception{
		this.editUserInSession(session);
		model= getMode(model,session,classNo,className,true);
	}
	/**
	...
	@ResponseBody 
	public VideoChild getLastNewVideo(HttpServletRequest req,HttpSession session,String gid,String lastVcid)throws Exception{
		Map<String,String> map=new HashMap<String, String>();
		   map.put("gid",gid);
		if(StringUtils.isNotEmpty(lastVcid)){
	...
	public void searchGeneralVideoList(HttpServletRequest req,HttpSession session,String gsbuject,String gclass,String gclassify,String sort,String nameType,Model model)throws Exception{
		User user = (User)session.getAttribute("user");
...
//		int pageSize = Integer.parseInt(pageLimit);
	 ...
	 * @param gclassify 年级下的分类（例：五年级:阅读）
	 * */
	@RequestMapping("/public/generalCourseList")
	public void getGeneralVideoList(HttpServletRequest req,HttpSession session,
			String gsbuject,String gclass,String gclassify,
			String sort,String nameType,Model model)throws Exception{
		User user = (User)session.getAttribute("user");
		Map<String,String>maps = new HashMap<String,String>();
		maps.put("gsbuject",gsbuject);
		maps.put("gclass",gclass);
...
//		int pageSize = Integer.parseInt(pageLimit);
	...
	public void searchGeneralVideoList_zt(HttpServletRequest req,HttpSession session,String gsbuject,String gclass,String gclassify,Model model)throws Exception{
		User user = (User)session.getAttribute("user");
		Map<String,String>maps = new HashMap<String,String>();
		maps.put("gsbuject",gsbuject);
		maps.put("gclass",gclass);
	 ...
	 * @param gclassify 年级下的分类（例：五年级:阅读）
	 * */
	@RequestMapping("/public/generalCourseList_zt")
	public void getGeneralVideoList_zt(HttpServletRequest req,HttpSession session,String gsbuject,String gclass,String gclassify,Model model)throws Exception{
		User user = (User)session.getAttribute("user");
		Map<String,String>maps = new HashMap<String,String>();
		maps.put("gsbuject",gsbuject);
		maps.put("gclass",gclass);
