...
public class BaseUtil {
	...
	public static String getWpayMOney(String money){
	   try{
	     ...
	     big=big.multiply(new BigDecimal("100"));//multiply 乘
   ...
   //获取秒杀相关信息
	public static Map<String,Object> getKillInfo(String killStartTime, String killEndTime){
		...
		// String showTimerText=(day>0?day+"天":"")+(hour>0?hour+"小时":"")+(minute>0?minute+"分":"")+(second>0?second+"秒":"");
		 String showTimerText=day+"天"+hour+"小时"+minute+"分"+second+"秒";
		 map.put("timeType", type);//活动状态
		 map.put("totalTime", totalTime);//活动时长
		 map.put("showTimerText", showTimerText);//显示的文字（例如：2天10小时30分22秒）
		 System.out.println("killStartTime:"+killStartTime+",killEndTime:"+killEndTime);
		 System.out.println("timeType:"+type+",totalTime:"+totalTime+",showTimerText:"+showTimerText);
		 return map;
		
	}
	
	//获取服务器路径
	public static String getServerPath(HttpServletRequest req,String dir,String filename, String uploadAbsolutePath){
...
//发送邮件
public static void sendEmail(String receiverEmail,String emailContent) throws Exception{
	 // 配置发送邮件的环境属性  
    ...
    // 发件人的账号  
    props.put("mail.user", "dongchengcheng@sylhxj.cn");  
    // 访问SMTP服务时需要提供的密码  
...
//获取当天日期指定格式字符串
public static String getCurrentDateStr(String formate) throws ParseException{
	 Date ddate=new Date();
...
public static String getPorpertites_pro(String key,String propertieName) throws IOException{
	propertieName=propertieName==null||propertieName.length()==0?"application.properties":propertieName;
