...
<div class="content">
    <div class="videobox">
    	<video src="${contestVideo.cvideoUrl}" id="myvideo" controls="controls" class="video">
    </div>
								...
								<div class="info_val info_val_active"> 
								   ...
								   <ul class="er" style="display:block;">
									   <li class="choose_li" style="word-break:break-all;word-wrap:break-word;">
									   	<c:if test="${empty contestVideo.cvideoUrl}">
									   		暂无视频!
									   	</c:if>
									   	<c:if test="${not empty contestVideo.cvideoUrl}">
							  ...
							  <li> 
								...
								<div class="info_val"> 
								   <ul class="er2" style="display:block;">
										<li><a id="testName" data-toggle="modal" data-target="#msk_test">本章测验题</a>  </li>
								   </ul>
								</div>
							  </li>
               ...
               <div class="every_info  thisc">
				<input class="cno" type="hidden" value="${c.cid}">
				 ...
				 <div class="classification-content-text">
					<h6>${c.cclassify}</h6>
					<p class="name" style="display:none;">${c.cname}</p>
				</p>
				</div>
				<p class="info_name_char">主讲老师：${c.teacherName}</p> 
				<p >
					<div class="clearfix rmbbox">
					    <c:if test="${c.cmoney=='0'}">
							<p class="rmb f_fl">免费</p>
						</c:if> 
						<c:if test="${c.cmoney!='0'}">
							<p class="rmb f_fl">￥${c.cmoney}</p>
						</c:if>
					    <span class="f_fr datebox" >时长：<span class="date"></span></span>
						<video src="${c.cvideoUrl }" controls class="videoHide" style="display:none">
						</video>
					</div>
				</p>
				<p class="study">
					<span>${c.cplayNo}</span>人学习
				</p>
			</div>
			</c:forEach>
            </div>
...
<c:if test="${!contestVideo.cmoney.equals('0')}"> 
   <div class="pay clearfix"> 
     ...
     <%--
       <p class="price">
         <c:if test="${not empty contestVideo.killName}">
          	【${contestVideo.killName}】
         </c:if> &nbsp;&nbsp; 
      ...
      --%> 
          <%--情况1--%>
           <c:if test="${killInfo.timeType==1 }"> 
            <c:if test="${isBuy==0}"> 
	            <p class="price">￥${contestVideo.killMoney}
		     	 <span>专柜价：￥${contestVideo.cmoney}</span> 
		       	</p>
...
<form action="" id="subFrom" method="post">
	...
	function payUseWeiPay(){//二维码支付.-->应该改为直接调用微信接口
		if(islogin=="0"){
  		  ...
  		  //发送请求到服务器生成预支付订单号
  			if(${contestVideo.cmoney } > 0){
  				var url="<%=basePath%>pay/1?vid=${contestVideo.cid}"; //注意此处的basePath是没有端口号的域名地址。如果包含:80,在提交给微信时有可能会提示 “redirect_uri参数错误” 。
...
<script src="${pageContext.request.contextPath}/jsp/js/jquery.1.10.1.min.js" type="text/javascript" charset="utf-8"></script>
	...
	<script type="text/javascript">
	 ...
	 var vid="${contestVideo.cid}";
		...
		function submitAnswer(){
			...
			$("#result_tests").show();
			//将学生成绩入库----- 2018年3月27日09:52:51  guodi 修过.!!! 待测试
			$.post(
				"${pageContext.request.contextPath}/back/addUserScore",
				{vid:vid,utype:1,score:totalScore,rightNum:right_num, errorNum:error_num,
				isHasJDT:isHasJDT,
				totalNums:$("#nums_hide").val(),
				totalScores:$("#scores_hide").val(),
				teacherName:"${contestVideo.teacherName}",
	...
	$(function(){
		initUnitTests(vid,1);//初始化章节测验html		
