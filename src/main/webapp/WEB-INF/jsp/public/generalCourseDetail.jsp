								...
								<div class="info_val info_val_active"> 
								   ...
								   <ul class="er" style="display:block;">
									   <li class="choose_li" style="word-break:break-all;word-wrap:break-word;">
									   	<c:if test="${empty generalVideo.gvideoUrl}">暂无视频!</c:if>
									   	<c:if test="${not empty generalVideo.gvideoUrl}">
	...
	function initPlayer(playerInstance,url,autostart){
	  ...
	  //初始化视频  
		  ...
		  //获取当前的播放时间  
		   ...
		   if(!isPlay){
				 $.post("${pageContext.request.contextPath}/video/changePlayCount",{vid:"${generalVideo.gid}",vclassify:0},function(){});
				 isPlay=true;
			  }
...
//				暂停当前播放视频
    ...
    function isRight(islogin){
	    if(islogin=="0"){
	    	...
	    	var rightContent="${user.rightContent}";//权限内容
...
<script src="${pageContext.request.contextPath}/jsp/js/jquery.1.10.1.min.js" type="text/javascript" charset="utf-8"></script>
	...
	function submitAnswer(){
		...
		$("#result_tests").show();
		//将学生成绩入库
		$.post("${pageContext.request.contextPath}/back/addUserScore",
				{vid:vid,utype:0,score:totalScore,rightNum:right_num,errorNum:error_num,
			isHasJDT:isHasJDT,totalNums:$("#nums_hide").val(),totalScores:$("#scores_hide").val(),
			teacherName:"${generalVideo.teacherName}",JDT_answer:JSON.stringify(JDT_answer)},
			function(res){});
